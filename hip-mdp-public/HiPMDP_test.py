"""
Modified version of hip-mdp-public/HiPMDP.py, so that we can evaluate on a single episode 
of the test instance.
Specifically, BNN updates, latent updates, and model-based DQN training are conducted 
all together during the single test episode
...
class HiPMDP_test(object):
	...
	def __init__(self, domain, preset_hidden_params, run_type='full', ddqn_learning_rate=0.0001, 
		     ...
		     grid_beta=0.23, print_output=False, config_domain={}):
		"""
	...
	def __initialize_params(self):
		...
		self.ddqn_batch_size = 10 # DDQN batch size
	...
	def __set_domain_hyperparams(self):
		...
		self.standardize_states = False
                self.steps_per_train = 10 # train DDQN once for every 10 real steps in the test instance
		# Acrobot settings
		if self.domain == 'acrobot':
			...
			self.bnn_start = 10 # number of time steps observed before starting BNN training
			self.dqn_start = 10 # number of time steps observed before starting DQN training
                        self.test_steps = 200
		# Grid settings
		...
		# HIV settings
			...
			raise NameError('invalid domain')
                self.approx_eps_per_train = int(self.num_approx_episodes / (self.test_steps / self.steps_per_train))
		...
		# Size of experience buffer for test instance. Note: all experiences are stored
                self.instance_buffer_size = self.max_task_examples * 1 # only one real episode allowed during test
		# Size of fictional experience buffer
                self.instance_fictional_buffer_size = min(100000, self.num_approx_episodes)
		if self.domain == 'grid':
	...
	def __load_reward_standardization(self):
		"""Load the reward mean and standard deviation."""
                with open('preset_parameters/%s'%self.config_domain['reward_std_filename'],'r') as f:
	...
	def __load_state_standardization(self):
		"""Load the state mean and standard deviation."""
                with open('preset_parameters/%s'%self.config_domain['state_std_filename'],'r') as f:
	...
	def __e_greedy_policy(self, state, eps=None):
		"""Select action using epsilon-greedy policy."""
                if eps is not None:
                        eps_local = eps
                else:
                        eps_local = self.eps
		if np.random.rand(1) < eps_local:
	...
	def run_instance(self):
		...
		while self.episode_iter < self.episode_count:
			self.run_episode()
                        if self.episode_iter == self.episode_count-1:
                                raw_input("Finished second real episode")
                                break
        ...
        def test_single_episode(self, idx_instance=0):
		...
		self.train_bnn = True
                ...
                fictional_exp_replay_param_set = {
                        'episode_count': self.num_approx_episodes * int(self.max_task_examples/5),
			'instance_count': 1,
			'max_task_examples': self.max_task_examples,
			'ddqn_batch_size': 10,
			'num_strata_samples': self.num_strata_samples,
			...
			'bnn_start': self.bnn_start
                ...
                self.fictional_buffer = ExperienceReplay(fictional_exp_replay_param_set, buffer_size=self.instance_fictional_buffer_size)
		self.instance_bnn_buffer = ExperienceReplay(bnn_exp_replay_param_set, buffer_type='BNN', buffer_size=self.instance_buffer_size)
                if self.standardize_rewards:
			if self.create_exp_batch:
				instance_idx = self.instance_iter
			else:
		...
		self.approx_steps = 0 # number of steps taking using approximated BNN rollout
                ...
                # Start running single episode on real environment
                self.task.reset(perturb_params = True, **self.instance_param_set)
		state = self.task.observe()
		if self.standardize_states:
			state = self.__standardize_state(state)
		...
		self.episode_buffer_bnn = []
		ep_reward = 0
                cumulative_reward = np.zeros(self.test_steps)
                while not self.task.is_done(self.max_task_examples) and self.total_steps < self.test_steps:
			self.total_steps += 1
                        print("------------ idx_instance %d | total_steps %d -----------\n" % (idx_instance, self.total_steps))
			self.instance_steps += 1
                        # use deterministic policy for running the real environment
			action = self.__e_greedy_policy(state, eps=0) 
			reward , next_state = self.task.perform_action(action, perturb_params=True, **self.instance_param_set)
			if self.standardize_rewards:
				reward = (reward-self.reward_mean) / self.reward_std
			if self.standardize_states:
				next_state = self.__standardize_state(next_state)
                        self.instance_bnn_buffer.add(np.reshape([np.reshape(np.array([state, self.__encode_action(action), reward,next_state, self.instance_iter]),[1,5])], [-1,5]))
			ep_reward += reward
                        cumulative_reward[self.total_steps-1] = ep_reward
			state = next_state

                        if self.instance_steps >= self.bnn_start and self.instance_steps % self.steps_per_train == 0:
                                print("Updating BNN")
                                for update_iter in xrange(self.num_bnn_updates):
				        self.__update_latent_weights()
                                self.__update_BNN()
                                                        
                        if self.instance_steps >= self.dqn_start and self.instance_steps % self.steps_per_train == 0:
                                print("Running fictional episodes")
                        	self.approx_episode_iter = 0
                                r_fictional = 0
                                while self.approx_episode_iter < self.approx_eps_per_train:
					r_fictional += self.run_fictional_episode()
					self.approx_episode_iter += 1
				        # Decay epsilon for the fictional episodes
				        if self.eps > self.eps_min:
					        self.eps *= self.eps_decay
                                print("Average fictional episode reward", r_fictional/float(self.approx_eps_per_train))

                if self.total_steps < self.test_steps:
                        remaining = np.ones(self.test_steps - self.total_steps) * ep_reward
                        cumulative_reward[self.total_steps : ] = remaining
