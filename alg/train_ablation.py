"""
Ablation SEPT-NP: no probe.
Instead, use trajectories generated by the control policy 
for the VAE to estimate z.
"""
import sys, os
import time
import pickle
...
def train_function(config, config_suffix=None):
    ...
    if domain == '2D':
        ...
        raise ValueError("train_ablation.py : domain not recognized")
    ...
    if use_pi_e:
        ...
        # initial z
        z_avg = np.zeros(config_VAE['n_latent'], dtype=np.float32)
        ...
        for idx_episode in range(1, N_episodes+1):
            ...
            for step in range(1, n_probe_steps+1):
                ...
                if use_pi_e:
                    ...
                    action = pi_c.run_actor(state, z_avg, sess, epsilon) # ablation
                ...
                if done and step < n_probe_steps:
                    ...
                    print("train_ablation.py : done is True while generating probe trajectory")
                    break
            ...
            if config_DDQN['activate']:
                ...
                # Update running mean of estimated z
                if not use_pi_e:
                    z_avg = 1/float(total_episodes) * z + (total_episodes - 1) * z_avg / float(total_episodes)
    ...
    # Save running mean of estimated z
    pickle.dump(z_avg, open('../results/%s/z_avg.p'%dir_name, 'wb'))
